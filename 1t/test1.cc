...
void chef(void* useless)
{
  ...
  cout << "Chef has started" << endl;

  assert(thread_lock(lock)!=-1);
  cout << "Chef has locked" << endl;
  cheflock:
  {
...
//   printf("Board is not full, chef is waiting for an order to be placed \n");
  cout << "Chef is going on wait " << chefCond << endl;
    thread_wait(lock, chefCond);
      cout << "Chef returned from wait " << chefCond << endl;
  }
...
//  printf("Chef is taking down order %i \n", next);
  ...
  numInUse = numInUse -1;
 cout << "READY: cashier " << (board+next)->id  << " sandwich " << (board+next)->sw << endl;
//  printf("READY: cashier %i sandwitch %i\n", (board+next)->id -2, (board+next)->sw);
  lastSw =(board+next)->sw;
//  printf("\n Shouting on id %i \n", (board+next)->id);
  cout << "Chef is broadcasting " << chefCond << endl;
...
void cashier(void* input)
{
  ...
  int sw = -1;
  int a = 0;
  cout << "Cashier " << id << " has started" << endl;
  ...
  assert(thread_lock(lock) !=-1);
    cout << "Cashier " << id << " has locked" << endl;
  if(livingCashiers==-1)
  {
    livingCashiers = 0;
  }
...
//  printf("Cashier %i has incremented %i \n", id,livingCashiers);
  ...
  while( a < 2)
  {
    ...
    //  printf("Cashier %i is waiting for a spot on the board \n", id);
      cout << "Cashier " << id << " going WaitOpen " << openCond << endl;
      thread_wait(lock, openCond);
        cout << "Cashier " << id << " returned WaitOpen "<< openCond << endl;
      assert(thread_lock(lock)==-1);
    ...
    //  printf("Cashier %i is checking board spot %i \n", id, i);
      ...
      //  printf("Cashier %i has found a spot at %i \n", id, i);
        ...
        (board+i)->id = id;
        (board+i)->sw = sw;
        cout << "POSTED: cashier " << id << " sandwich " << sw << endl;
      //  printf("POSTED: cashier %i sandwitch %i\n", id, sw);
            cout << "Cashier " << id << " broadcasted " << chefCond << endl;
        thread_broadcast(lock, chefCond);
        while((board+i)->inuse)
        {
          //cout << "4" << endl;
          cout << "Cashier " << id << " going WaitSW " << id << endl;
          thread_wait(lock, id);
            cout << "Cashier " << id << " returned WaitSW "<< id << endl;
        //  printf("\n Heard on %i \n", id);
        }
        assert(!((board+i)->inuse));
        cout << "Cashier " << id << " broadcasted " << openCond << endl;
 ...
 //printf("Cashier %i has left the building, Living %i \n", id, livingCashiers);
   cout << "Cashier " << id << " broadcasted " << openCond << " and " << chefCond << endl;
 thread_broadcast(lock, openCond);
 thread_broadcast(lock, chefCond);
         cout << "Cashier " << id << " unlocking" << endl;
